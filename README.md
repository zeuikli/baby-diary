...
| 👪 家人協作 | 跨裝置自動同步寶寶清單 + 一鍵分享設定碼（複製 / 貼上視覺回饋）；加密功能需 HTTPS 安全連線，不支援時自動顯示設定碼供手動複製 |
...
>
> ⚠️ 設定碼加密（AES-GCM）使用瀏覽器 Web Crypto API，**需要 HTTPS 或 localhost** 才能正常運作。若 Clipboard API 被瀏覽器拒絕，會自動顯示設定碼文字框供手動複製。
