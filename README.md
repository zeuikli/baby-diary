...
| 📏 成長曲線 | 體重 / 身高 / 頭圍紀錄、**WHO 0-5 歲國際生長標準百分位曲線 (P3 / P50 / P97)**、每項最新數值獨立顯示 |
| 📖 日記 | 自由文字記事、每日心情紀錄 |
| 📊 統計 | 多日趨勢圖 |
| 📥 匯入 | 從既有資料批次匯入，具進度條 |
...
- 以**月齡（0–60 個月）為 X 軸**，寶寶實際數據疊加在 P3 / P50 / P97 參考虛線之上
- 「最新數值」分別以 **體重 / 身高 / 頭圍** 各自最近一筆非空紀錄顯示，每項各自帶上自己的日期（避免只更新單一項目時其他欄位被蓋掉）
- WHO 參考資料來源：World Health Organization Child Growth Standards（0-5 歲國際嬰幼兒生長標準）
...
1. `npm ci` 安裝依賴
2. `npm run build` 產出 `dist/`
3. 使用 `peaceiris/actions-gh-pages` 將 `dist/` 發佈到 `gh-pages` 分支
