...
| 類別 | 功能 |
| --- | --- |
| 📱 首頁 | 寶寶頭像 / 姓名 / 出生天數、今日摘要卡、進行中睡眠計時器、快速記錄按鈕 × 6、日期選擇器（點擊跳任意日期 / 左右箭頭逐日 / 「今」鍵回今天）、時間軸 |
...
| `d9af845` | **feat(home): 日期選擇器改為可點擊跳轉任意日期** — 點擊日期文字開啟原生日曆、非今天時顯示「今」按鈕一鍵回今天，左右箭頭逐日切換保留 |
| `2e1d379` | **feat(home): 首頁摘要卡新增擠奶、副食品 + 新建副食品列表頁** — 摘要卡從 3 個擴充為 5 個，新增 `/solids` 頁面含完整 CRUD |
| `9a0eb2a` | **fix(home): 修正擠奶導向、副食品標題、時間軸可直接編輯紀錄** — 擠奶卡改導向新建的 `/pumping` 頁面，時間軸點擊紀錄傳入 `editRecord` 進入編輯模式 |
| `f96a122` | **feat: 擠奶/副食品頁面標題 + 副食品新增食量欄位** — Header 加入擠奶紀錄 / 副食品紀錄標題，副食品新增「食量 (ml)」輸入並同步匯出匯入 |
| `bf6b9a8` | **fix: 統一所有 modal 按鈕文字 — 編輯時顯示「更新紀錄」** — QuickDiaperModal 原本寫死「儲存記錄」，四個 modal 統一為 editRecord ? 更新紀錄 : 儲存紀錄 |
| `a0be4c9` | **fix(growth): 百分位說明文字修正為 0-2 歲** — 與圖表實際顯示範圍 0-24 個月一致 |
